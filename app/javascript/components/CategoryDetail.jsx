...
const CategoryDetail = ({ categoryId }) => {
  ...
  useEffect(() => {
    const fetchCategoryData = async () => {
      try {
        ...
        const categoryResponse = await axios.get(`/categories/${categoryId}/show_react.json`);
        setCategory(categoryResponse.data.category);

        // Fetch books in this category
        const booksResponse = await axios.get(`/books.json?category_id=${categoryId}`);
  ...
  const handleEdit = () => {
    window.location.href = `/categories/${categoryId}/edit_react`;
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-2 text-gray-600">Detalles de la categoría</p>
          </div>
      ...
      <div className="bg-white shadow-sm rounded-lg p-6 mb-8">
        <h2 className="text-lg font-medium text-gray-900 mb-4">Información de la Categoría</h2>
        <dl className="grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-2">
          ...
          <div>
            ...
            <dd className="mt-1 text-sm text-gray-900">{category.description || 'No especificada'}</dd>
          </div>
          <div>
          ...
          <div>
            <dt className="text-sm font-medium text-gray-500">Libros disponibles</dt>
      ...
      <div className="bg-white shadow-sm rounded-lg p-6">
        <h2 className="text-lg font-medium text-gray-900 mb-4">
          Libros en esta categoría ({books.length})
        </h2>
        ...
        {books.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="mt-1 text-sm text-gray-500">Esta categoría aún no tiene libros registrados.</p>
          </div>
        ) : (
            ...
            {books.map((book) => (
              <div key={book.id} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                ...
                <p className="text-xs text-gray-500 mb-2">
                  Autor: {book.author_name || 'No especificado'}
                </p>
